...
觸發重整後，會自動「送出重整網址 → 等待 → 再檢查」重複最多 `MAX_RETRIES` 次（預設 1 次，
也就是只重整一次；因為 fxtwitter 翻譯品質本身不受我們控制，重試太多次只會一直拿到同樣
爛的翻譯、在頻道洗版）。如果重試次數用完了還是失敗，會發一則警告到 `ALERT_CHANNEL_ID`，
提醒你自己看一下那則推文，而不是默默放棄。

如果重試不只一次，從第二次重整開始，每次要送出新的重整連結前，會先把「吹雪自己上一次
發的」重整卡片刪掉，讓重試訊息彼此取代、不會疊出好幾張卡片洗版；染岡原本發的第一則貼文
永遠不會被刪。因為只刪自己發過的訊息，不需要額外的「管理訊息」權限。
